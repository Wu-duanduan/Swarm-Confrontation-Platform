...
def get_args():
    ...
    parser.add_argument("--model-dir", type=str, default="saved_models", help="模型文件所在目录")
    parser.add_argument("--model-prefix", type=str, default="best_dqn_model_agent_qmix_", help="模型文件名前缀")
...
def test_model(args, obsCenter):
    ...
    # 加载每个智能体的预训练模型
    for agent_id in range(blue_agents):
        model_path = os.path.join(args.model_dir, f"{args.model_prefix}{agent_id}.pth")
        try:
            trainer.agents[agent_id].load_model(model_path=model_path, agent_id=agent_id)
            print(f"成功加载智能体 {agent_id} 的模型: {model_path}")
        except Exception as e:
            print(f"智能体 {agent_id} 加载模型失败: {e}")
            print(f"智能体 {agent_id} 将使用随机初始化的模型进行测试")

    # 加载混合网络
    try:
        trainer.load_mixing_network()
        print("成功加载混合网络: saved_models/best_qmix_mixing_network.pth")
    except Exception as e:
        print(f"混合网络加载失败: {e}")
        print("将使用随机初始化的混合网络")
    ...
    # 保存结果到文本文件
    file_path = os.path.join("test_results", "best_dqn_model_qmix.txt")
