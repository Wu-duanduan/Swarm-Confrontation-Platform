...
import os
import sys
from Network.COMA_network import MultiAgentCOMA
...
def get_args():
    ...
    parser.add_argument("--model-prefix", type=str, default="best_dqn_model_agent_coma_", help="模型文件名前缀")
...
def test_model(args, obsCenter):
    ...
    dqn_params = {
        ...
        'state_dim': iifds.numberofcar * 9
    }

    # 初始化多智能体DQN强化学习
    trainer = MultiAgentCOMA(blue_agents, dqn_params)

    # 加载每个智能体的预训练模型
    for agent_id in range(blue_agents):
        model_path = os.path.join(args.model_dir, f"{args.model_prefix}{agent_id}.pth")
        try:
            trainer.agents[agent_id].load_model(model_path=model_path)
    ...
    try:
        trainer.load_critic()
        print("成功加载混合网络: saved_models/best_coma_critic.pth")
    except Exception as e:
        print(f"混合网络加载失败: {e}")
        print("将使用随机初始化的混合网络")
    ...
    for episode in range(num_episodes):
        ...
        for step in range(max_steps):
            ...
            # 获取动作并更新任务目标
            actions, _ = trainer.get_actions(obs, eval_mode=True)
    ...
    # 保存结果到文本文件
    file_path = os.path.join("test_results", "best_dqn_model_coma.txt")
