...
import pandas as pd
import sys
from Network.QMIX_network import MultiAgentDQN
...
def get_args():
    ...
    parser.add_argument("--state-dim", type=int, default=iifds.numberofcar * 9, help="Global state dimension")
...
if __name__ == "__main__":
    ...
    reward_list = []  # 记录每回合奖励
    print(f"blue_agents: {blue_agents}, expected: {int(iifds.numberofcar / 2)}")
    # ===========================
    # 初始化动力学约束以及随机参数（马子豪）
    # ===========================
    random_params = {
    ...
    dqn_params = {
        ...
        'state_dim': args.state_dim  # 添加全局状态维度
    }
    print(f"batch_size: {dqn_params['batch_size']}, state_dim: {dqn_params['state_dim']}")
    # 初始化多智能体DQN强化学习
    ...
    for episode_gone in range(max_episode):
        ...
        for step in range(max_step):
            ...
            # 获取动作并更新任务目标
            actions = trainer.get_actions(obs, eval_mode=False)  # 使用评估模式
            ...
            next_state = np.concatenate([
                np.array(qNext).flatten(),
                np.array(vNext).flatten(),
                HP_index,
                flag_car,
                missle_index
            ])
            ...
            if not (episode_gone == 0 and step == 0):
                ...
                for car_id in range(blue_agents):
                    if flag_car[car_id] == 0:
                        experiences.append(
                            (obs[car_id], actions[car_id], global_reward, next_obs[car_id], global_done))
                    else:
                        placeholder_obs = np.zeros(dqn_params['obs_dim'])
                        placeholder_action = 0
                        placeholder_next_obs = np.zeros(dqn_params['obs_dim'])
                        experiences.append(
                            (placeholder_obs, placeholder_action, global_reward, placeholder_next_obs, global_done))
                trainer.store_experiences(experiences, state, next_state)
                loss = trainer.train()
                if loss is not None:
        ...
        if total_reward > best_reward:
            ...
            for agent in trainer.agents:
                agent.save_model(episode=episode_gone, reward=total_reward)
            trainer.save_mixing_network(episode=episode_gone, reward=total_reward)
            print(f"新的最佳模型已为所有智能体和混合网络保存，奖励: {best_reward:.2f}")
