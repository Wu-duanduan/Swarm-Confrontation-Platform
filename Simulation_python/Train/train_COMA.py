...
if __name__ == "__main__":
    ...
    for episode_gone in range(max_episode):
        ...
        for step in range(max_step):
            ...
            # 获取动作并更新任务目标
            actions, action_probs = trainer.get_actions(obs, eval_mode=False)
            ...
            if not (episode_gone == 0 and step == 0):
                ...
                for car_id in range(blue_agents):
                    if flag_car[car_id] == 0:
                        experiences.append(
                            (obs[car_id], actions[car_id], rewards[car_id], next_obs[car_id], blue_dones[car_id]))
                trainer.store_experiences(experiences, state, next_state, action_probs)
                # 训练
                loss = trainer.train()
                if loss is not None:
        ...
        if total_reward > best_reward:
            ...
            trainer.save_critic(episode=episode_gone, reward=total_reward)
            print(f"新的最佳模型已为所有智能体和评论家保存，奖励: {best_reward:.2f}")
